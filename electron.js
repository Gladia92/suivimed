...
ipcMain.handle("ollama-analyze", async (_e, prompt) => {
  const resp = await fetch("http://127.0.0.1:11434/api/chat", {
    ...
    body: JSON.stringify({
      ...
      stream: false,
      messages: [
        { role: "system", content: "Tu es un pharmacien clinicien et médecin. Tu analyses un journal de prises de médicaments (observance) afin d'aider le médecin traitant. Réponds UNIQUEMENT en français, de façon structurée (titres en majuscules + tirets), factuelle et prudente. Commence toujours par le titre exact « COMPTE RENDU OBSERVANCE POUR LE MÉDECIN TRAITANT » (respecte cette casse, ne le reformule pas). Compare les prises réelles à la posologie prescrite, signale les écarts (oublis, surconsommation/surdosage, sous-dosage), les risques d'interaction au vu du profil et du traitement de fond. Tu peux SUGGÉRER au médecin des pistes d'ajustement ou des alternatives thérapeutiques À DISCUTER, mais en rappelant explicitement que ce ne sont que des suggestions destinées au médecin et JAMAIS une prescription. Ne recopie pas la consigne, n'invente aucune donnée, fonde-toi strictement sur les informations fournies." },
